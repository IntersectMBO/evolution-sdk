...
/**
 * Coin Selection Failure Tests
 * 
 * These tests specifically prove that coin selection can fail
 * when there are insufficient funds or missing required assets.
 * 
 * Failure scenarios:
 * 1. Insufficient lovelace - not enough ADA to cover payment + fees
 * 2. Missing native assets - required token not in any available UTxO
 * 3. Insufficient native assets - token exists but quantity too low
 * 4. Mixed insufficiency - some assets available, others missing
 */
...
// Using shared test helper from test/utils/utxo-helpers.ts

// ============================================================================
// Test Suite: Coin Selection Failures
// ============================================================================

describe.concurrent("TxBuilder Coin Selection Failures", () => {
  ...
  describe("Missing Native Assets", () => {
    ...
    it("should fail when required native asset does not exist in any UTxO", async () => {
      ...
      const utxos: Array<UTxO.UTxO> = [
        createTestUtxo({ txHash: "tx1", outputIndex: 0, address: CHANGE_ADDRESS, lovelace: 10_000_000n, nativeAssets: { [tokenA]: 1000n } }),
      ]
  ...
  describe("Insufficient Native Asset Quantity", () => {
    ...
    it("should fail when token exists but quantity is too low", async () => {
      ...
      const utxos: Array<UTxO.UTxO> = [
        createTestUtxo({ txHash: "tx1", outputIndex: 0, address: CHANGE_ADDRESS, lovelace: 10_000_000n, nativeAssets: { [tokenA]: 50n } }),
      ]
    ...
    it("should fail when tokens are fragmented across UTxOs but total is insufficient", async () => {
      ...
      const utxos: Array<UTxO.UTxO> = [
        createTestUtxo({ txHash: "tx1", outputIndex: 0, address: CHANGE_ADDRESS, lovelace: 5_000_000n, nativeAssets: { [tokenA]: 30n } }),
        createTestUtxo({ txHash: "tx2", outputIndex: 0, address: CHANGE_ADDRESS, lovelace: 5_000_000n, nativeAssets: { [tokenA]: 30n } }),
        createTestUtxo({ txHash: "tx3", outputIndex: 0, address: CHANGE_ADDRESS, lovelace: 5_000_000n, nativeAssets: { [tokenA]: 30n } }),
      ]
  ...
  describe("Complex Mixed Failures", () => {
    ...
    it("should fail when UTxOs exist but all are too small for min UTxO + fees", async () => {
      ...
      const utxos: Array<UTxO.UTxO> = Array.from({ length: 10 }, (_, i) =>
        createTestUtxo({ txHash: `tx${i}`, outputIndex: 0, address: CHANGE_ADDRESS, lovelace: 1000n }) // 0.001 ADA each
      )
    ...
    it("should fail when combined shortfalls across lovelace and multiple assets", async () => {
      ...
      const utxos: Array<UTxO.UTxO> = [
        createTestUtxo({ txHash: "tx1", outputIndex: 0, address: CHANGE_ADDRESS, lovelace: 500_000n, nativeAssets: { 
          [tokenA]: 10n,  // Need 100
          [tokenB]: 5n    // Need 50
        } }),
      ]
