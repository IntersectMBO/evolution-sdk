...
describe("Largest-First Coin Selection", () => {
  ...
  describe("Basic Selection", () => {
    it("should select single UTxO when sufficient", () => {
      const availableUtxos: ReadonlyArray<UTxO.UTxO> = [createCoreTestUtxo({ lovelace: 10_000_000n, index: 0 })]

      const requiredAssets = Assets.fromLovelace(5_000_000n)

      const result = largestFirstSelection(availableUtxos, requiredAssets)

      expect(result.selectedUtxos).toHaveLength(1)
      expect(Assets.lovelaceOf(result.selectedUtxos[0].assets)).toBe(10_000_000n)
    })
    ...
    it("should select multiple UTxOs when single is insufficient", () => {
      ...
      const requiredAssets = Assets.fromLovelace(5_500_000n)

      const result = largestFirstSelection(availableUtxos, requiredAssets)

      // Should select largest first (3M + 2M + 1M = 6M >= 5.5M)
      expect(result.selectedUtxos).toHaveLength(3)
      expect(Assets.lovelaceOf(result.selectedUtxos[0].assets)).toBe(3_000_000n)
      expect(Assets.lovelaceOf(result.selectedUtxos[1].assets)).toBe(2_000_000n)
      expect(Assets.lovelaceOf(result.selectedUtxos[2].assets)).toBe(1_000_000n)
    })
    ...
    it("should select exactly enough UTxOs (stops when requirements met)", () => {
      ...
      const requiredAssets = Assets.fromLovelace(12_000_000n)
      ...
      expect(Assets.lovelaceOf(result.selectedUtxos[0].assets)).toBe(10_000_000n)
      expect(Assets.lovelaceOf(result.selectedUtxos[1].assets)).toBe(5_000_000n)
    })
  })
  ...
  describe("Sorting Behavior", () => {
    it("should select largest UTxOs first", () => {
      ...
      const requiredAssets = Assets.fromLovelace(8_000_000n)
    ...
    it("should maintain stable sort for equal lovelace values", () => {
      ...
      expect(result.selectedUtxos).toHaveLength(3)
      expect(result.selectedUtxos.every((u) => Assets.lovelaceOf(u.assets) === 5_000_000n)).toBe(true)
    })
  })
  ...
  describe("Multi-Asset Selection", () => {
    it("should select UTxOs covering multiple required assets", () => {
      ...
      const requiredAssets = Assets.addByHex(Assets.fromLovelace(7_000_000n), policyId, assetNameHex, 120n)
      ...
      const totalAssets = result.selectedUtxos.reduce((acc, u) => Assets.merge(acc, u.assets), Assets.zero)

      expect(Assets.lovelaceOf(totalAssets)).toBeGreaterThanOrEqual(7_000_000n)
    })
    ...
    it("should handle UTxOs with different native assets", () => {
      ...
      let requiredAssets = Assets.fromLovelace(10_000_000n)
      requiredAssets = Assets.addByHex(requiredAssets, policyA, assetNameA, 120n)
      requiredAssets = Assets.addByHex(requiredAssets, policyB, assetNameB, 250n)
      ...
      expect(Assets.lovelaceOf(totalAssets)).toBeGreaterThanOrEqual(10_000_000n)
    })
  })
  ...
  describe("Error Handling", () => {
    ...
    it("should throw CoinSelectionError with detailed info on insufficient funds", () => {
      const availableUtxos: ReadonlyArray<UTxO.UTxO> = [createCoreTestUtxo({ lovelace: 2_000_000n, index: 0 })]

      const requiredAssets = Assets.fromLovelace(10_000_000n)
    ...
    it("should throw CoinSelectionError when insufficient native assets", () => {
      ...
      const requiredAssets = Assets.addByHex(Assets.fromLovelace(5_000_000n), policyId, assetNameHex, 100n)
  ...
  describe("Edge Cases", () => {
    it("should handle empty UTxO list", () => {
      ...
      const requiredAssets = Assets.fromLovelace(1_000_000n)
    ...
    it("should handle zero required lovelace", () => {
      ...
      const requiredAssets = Assets.addByHex(Assets.fromLovelace(0n), policyId, assetNameHex, 50n)
    ...
    it("should handle UTxO with zero lovelace but has native assets", () => {
      ...
      const requiredAssets = Assets.addByHex(Assets.fromLovelace(2_000_000n), policyId, assetNameHex, 50n)
    ...
    it("should handle exact match (no change needed)", () => {
      const availableUtxos: ReadonlyArray<UTxO.UTxO> = [createCoreTestUtxo({ lovelace: 5_000_000n, index: 0 })]
      ...
      expect(result.selectedUtxos).toHaveLength(1)
      expect(Assets.lovelaceOf(result.selectedUtxos[0].assets)).toBe(5_000_000n)
    })
    ...
    it("should handle very large lovelace values", () => {
      ...
      const requiredAssets = Assets.fromLovelace(1_000_000_000_000n) // 1 million ADA

      const result = largestFirstSelection(availableUtxos, requiredAssets)

      expect(result.selectedUtxos).toHaveLength(1)
      expect(Assets.lovelaceOf(result.selectedUtxos[0].assets)).toBe(45_000_000_000_000n)
    })
  })
  ...
  describe("Algorithm Optimization", () => {
    ...
    it("should not select more UTxOs than necessary", () => {
      ...
      const requiredAssets = Assets.fromLovelace(25_000_000n)
      ...
      const totalAssets = result.selectedUtxos.reduce((acc, u) => Assets.merge(acc, u.assets), Assets.zero)
      expect(Assets.lovelaceOf(totalAssets)).toBe(30_000_000n)
    })
  })
  ...
  describe("Integration Scenarios", () => {
    it("should work with realistic wallet UTxO set", () => {
      ...
      const requiredAssets = Assets.fromLovelace(30_000_000n) // Need 30 ADA

      const result = largestFirstSelection(availableUtxos, requiredAssets)

      // Should select 50 ADA UTxO only
      expect(result.selectedUtxos).toHaveLength(1)
      expect(Assets.lovelaceOf(result.selectedUtxos[0].assets)).toBe(50_000_000n)
    })
    ...
    it("should handle transaction with fees scenario", () => {
      ...
      const requiredAssets = Assets.fromLovelace(10_200_000n) // 10 ADA output + 0.2 ADA fee

      const result = largestFirstSelection(availableUtxos, requiredAssets)

      // Should select 15 ADA UTxO
      expect(result.selectedUtxos).toHaveLength(1)
      expect(Assets.lovelaceOf(result.selectedUtxos[0].assets)).toBe(15_000_000n)
    })
  })
})
