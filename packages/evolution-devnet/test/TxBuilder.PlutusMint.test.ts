...
describe("TxBuilder Plutus Minting (Devnet Submit)", () => {
  ...
  const createTestClient = () =>
    client(Cluster.getChain(devnetCluster!)).with(kupmios({ kupoUrl: "http://localhost:1444", ogmiosUrl: "http://localhost:1339" })).with(seedWallet({ mnemonic: TEST_MNEMONIC, accountIndex: 0 }))
