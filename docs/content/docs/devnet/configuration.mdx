...
const sdkClient = client(Cluster.BOOTSTRAP_CHAIN)
  .with(seedWallet({
...
const client1 = client(Cluster.BOOTSTRAP_CHAIN)
  .with(seedWallet({
mnemonic: "your mnemonic here",
    accountIndex: 0
}));

const client2 = client(Cluster.BOOTSTRAP_CHAIN)
  .with(seedWallet({
mnemonic: "your mnemonic here",
    accountIndex: 1 // Different account index = different address
}));

const address1 = Address.toHex(await client1.getAddress());
const address2 = Address.toHex(await client2.getAddress());
...
const addr1 = Address.toHex(await wallet1.getAddress());
const addr2 = Address.toHex(await wallet2.getAddress());
...
**Client chain parameter**: Always use `Cluster.BOOTSTRAP_CHAIN` for devnet clients to create testnet-format addresses (addr_test...).
