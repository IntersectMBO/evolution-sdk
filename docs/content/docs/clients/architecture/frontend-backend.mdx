...
- **Frontend**: User wallets for signing (API wallet sdkClient)
- **Backend**: Transaction building with provider access (Read-only sdkClient)
...
baseUrl: "https://cardano-mainnet.blockfrost.io/api/v0",
  ...
  .with(readOnlyWallet(userAddressBech32));
...
const witnessSet = await sdkClient.signTx(txCbor);
```
...
await sdkClient.sign(data); // Error: Cannot sign with read-only wallet
```
