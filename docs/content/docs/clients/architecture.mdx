...
// Backend: Create provider sdkClient, then attach read-only wallet
...
const backendClient = providerClient.with(readOnlyWallet("addr1qz8eg0aknl96hd3v6x3qfmmz5zhtrq5hn8hmq0x4qd6m2qdppx88rnw3eumv9zv2ctjns05c8jhsqwg98qaxcz2qh45qhjv39c"));
...
const address = await sdkClient.getAddress();
...
// 6. Submit via wallet
const txHash = await sdkClient.walletSubmitTx(unsignedTxCbor);
...
baseUrl: "https://cardano-mainnet.blockfrost.io/api/v0",
  ...
  .with(readOnlyWallet(userAddress));
...
async function sendPayment(recipientAddress: string, lovelace: bigint) {
  ...
  const walletApi = await cardano.eternl.enable();
  const walletClient = client(mainnet)
  .with(cip30Wallet(walletApi));

  // 2. Get user address (returns Core Address, convert to bech32 for backend)
  const userAddress = Address.toBech32(await walletClient.getAddress());
  ...
  // 5. Submit via wallet
  const txHash = await walletClient.walletSubmitTx(txCbor);
...
baseUrl: "https://cardano-mainnet.blockfrost.io/api/v0",
  ...
  const userClient = providerClient.with(readOnlyWallet(userAddressBech32));

  // Build unsigned transaction
  const builder = userClient.newTx();
  builder.payToAddress({
...
|--------|------|-------------|------------|-----------|
| `getAddress()` | Yes | Yes | Yes | Yes |
| `getRewardAddress()` | Yes | Yes | Yes | Yes |
...
const builder = sdkClient.newTx(); // Error: Cannot build without provider
```
...
const witnessSet = await sdkClient.signTx(txCborFromBackend);
```
...
await sdkClient.sign(); // Error: Cannot sign with read-only wallet
```
