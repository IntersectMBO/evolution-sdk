...
graph LR
    ...
    Type -->|blockfrost| BF[baseUrl + projectId]
    Type -->|kupmios| KP[kupoUrl + ogmiosUrl]
    Type -->|maestro| MA[network + apiKey]
    Type -->|koios| KO[baseUrl + token?]
...
**Client Configuration**: Provider is attached via `.with(provider)` on the composable client. The client stores the provider reference and passes it to transaction builders.
