...
<Mermaid chart={`
stateDiagram-v2
    [*] --> BaseClient: client(chain)
    
    BaseClient --> ProviderClient: .with(provider)
    BaseClient --> WalletClient: .with(wallet)
    BaseClient --> FullClient: .with(provider).with(wallet)
    
    ProviderClient --> FullClient: .with(wallet)
    WalletClient --> FullClient: .with(provider)
    
    FullClient --> [*]
    
    BaseClient: [1] Base Client
    ProviderClient: [2] Provider Client
    WalletClient: [3] Wallet Client
    FullClient: [4] Full Client
`} />

**[1] Base Client**: Chain context only. No blockchain access or signing capability. Created by `client(chain)`.

**[2] Provider Client**: Can query blockchain and submit transactions, cannot sign. Created by `.with(provider)`.

**[3] Wallet Client**: Can sign transactions and messages, must attach provider to build transactions. Created by `.with(wallet)`.

**[4] Full Client**: Either read-only (query + unsigned transactions) or signing (query + sign + submit) depending on wallet type. Created by chaining `.with(provider)` and `.with(wallet)`.
