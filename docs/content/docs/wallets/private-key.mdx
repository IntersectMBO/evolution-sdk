...
```typescript twoslash
import { preprod, client as makeClient } from "@evolution-sdk/evolution"
...
```typescript twoslash
import { mainnet, client as makeClient } from "@evolution-sdk/evolution"
...
```typescript twoslash
import { preprod, mainnet, client as makeClient } from "@evolution-sdk/evolution"

// Development
const devClient = makeClient(preprod)
  .withPrivateKey({
    paymentKey: await loadFromVault("dev/cardano-payment-key")
  })

// Staging
const stagingClient = makeClient(preprod)
  .withPrivateKey({
    paymentKey: await loadFromVault("staging/cardano-payment-key") // Different key
  })

// Production
const prodClient = makeClient(mainnet)
  .withPrivateKey({
