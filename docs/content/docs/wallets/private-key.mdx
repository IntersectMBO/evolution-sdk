...
```typescript twoslash
import { client, preprod, blockfrost, privateKeyWallet } from "@evolution-sdk/evolution";
...
async function example() {
  ...
  // const fullClient = sdkClient.with(blockfrost({ ... }));
}
```
...
paymentKey: await loadFromVault("cardano-payment-key-new")
  ...
  await monitorTransactions(sdkClient);
...
```typescript twoslash
import { client, preprod, mainnet, blockfrost, privateKeyWallet } from "@evolution-sdk/evolution";

async function createEnvironmentClients() {
  // Development
  const devClient = client(preprod)
    .with(privateKeyWallet({
      paymentKey: await loadFromVault("dev/cardano-payment-key")
    }));

  // Staging
  const stagingClient = client(preprod)
    .with(privateKeyWallet({
      paymentKey: await loadFromVault("staging/cardano-payment-key")  // Different key
    }));

  // Production
  const prodClient = client(mainnet)
    .with(privateKeyWallet({
      paymentKey: await loadFromVault("prod/cardano-payment-key")  // Different key
    }));

  return { devClient, stagingClient, prodClient };
}
