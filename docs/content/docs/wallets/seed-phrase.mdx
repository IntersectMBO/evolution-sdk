...
// const fullClient = sdkClient.with(blockfrost({ baseUrl: "...", projectId: "..." }));
```
...
// Testnet client
const testClient = client(preprod)
  .with(seedWallet({
mnemonic: "fitness juice ankle box prepare gallery purse narrow miracle next soccer category analyst wait verb patch kit era hen clerk write skin trumpet attract",
    accountIndex: 0
}));
...
const mainClient = client(mainnet)
  .with(seedWallet({
...
mnemonic: process.env.TEST_WALLET_MNEMONIC!,
  ...
  it("should sign transaction", async () => {
    ...
    // For full flow, attach provider: sdkClient.with(blockfrost({ baseUrl: "...", projectId: "..." }))
    const address = await sdkClient.getAddress();
