...
async function signAndSubmit() {
  ...
  // 3. Get user address
  const userAddress = await sdkClient.getAddress();
  ...
  // 6. Submit via wallet API (CIP-30 wallets can submit)
...
async function connectAndPay() {
  ...
  // Submit via wallet API (CIP-30 wallets can submit)
  const txHash = await sdkClient.walletSubmitTx(txCbor);
