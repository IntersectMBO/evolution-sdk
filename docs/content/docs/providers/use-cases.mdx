...
export async function trackDistribution(
  ...
  for (const addressBech32 of addresses) {
    ...
    const utxos = await sdkClient.getUtxosWithUnit(address, tokenUnit);
...
export async function trackNFTCollection(
  ...
  const ownership = await Promise.all(
    tokenNames.map(async (tokenName) => {
      ...
      try {
        const utxo = await sdkClient.getUtxoByUnit(unit);
...
export async function trackPoolDelegators(
  ...
  const delegators = await Promise.all(
    rewardAddresses.map(async (rewardAddress) => {
      try {
        const delegation = await sdkClient.getDelegation(RewardAddress.RewardAddress.make(rewardAddress));
...
```typescript twoslash
import { Address, Chain, PoolKeyHash, RewardAddress, Transaction, TransactionHash, client, mainnet, blockfrost, kupmios, koios, maestro } from "@evolution-sdk/evolution";
...
class ProviderWithFallback {
  ...
  constructor(
    ...
    this.clients = configs.map(({ chain, provider }) => {
      const providerInstance =
        provider.type === "blockfrost" ? blockfrost({ baseUrl: provider.baseUrl, projectId: provider.projectId })
        : provider.type === "maestro" ? maestro({ baseUrl: provider.baseUrl, apiKey: provider.apiKey })
        : provider.type === "kupmios" ? kupmios({ ogmiosUrl: provider.ogmiosUrl, kupoUrl: provider.kupoUrl })
        : koios({ baseUrl: provider.baseUrl });
      return client(chain).with(providerInstance);
    });
  }
  ...
  async getUtxos(address: Address.Address) {
    return this.withFallback((sdkClient) => sdkClient.getUtxos(address));
  }
  
  async submitTx(tx: Transaction.Transaction) {
    return this.withFallback((sdkClient) => sdkClient.submitTx(tx));
  }
  
  async getProtocolParameters() {
    return this.withFallback((sdkClient) =>
      sdkClient.getProtocolParameters()
    );
  }
}
