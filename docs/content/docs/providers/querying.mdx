...
// Query by credential
const utxos = await sdkClient.getUtxos(credential);
```
...
const utxosWithToken = await sdkClient.getUtxosWithUnit(
  Address.fromBech32("addr1qxy8sclc58rsck0pzsc0v4skmqjwuqsqpwfcvrdldl5sjvvhyltp7fk0fmtmrlnykgmhnzcns2msa2cmpvllzgqd2azqhpv8e4"),
  unit
);
...
const utxos = await sdkClient.getUtxosByOutRef([
...
const utxo = await sdkClient.getUtxoByUnit(nftUnit);
...
const datumCbor = await sdkClient.getDatum(datumHash);
...
async function batchQuery(addressesBech32: string[]) {
  ...
  const results = await Promise.all(
    addressesBech32.map(addr => 
      sdkClient.getUtxos(Address.fromBech32(addr))
    )
  );
...
type ProtocolParams = Awaited<ReturnType<typeof sdkClient.getProtocolParameters>>;
...
async function getCachedProtocolParameters() {
  ...
  if (!cachedParams || now - cacheTime > CACHE_DURATION) {
    cachedParams = await sdkClient.getProtocolParameters();
    cacheTime = now;
  }
