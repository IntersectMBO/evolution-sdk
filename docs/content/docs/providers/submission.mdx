...
```typescript twoslash
import { Transaction, TransactionHash, client, mainnet, blockfrost } from "@evolution-sdk/evolution";
...
const confirmed = await sdkClient.awaitTx(txHash, 10000);
...
const redeemers = await sdkClient.evaluateTx(unsignedTx);
...
export async function submitAndWait(
  ...
  try {
    ...
    const confirmed = await sdkClient.awaitTx(txHash, checkInterval);
...
async function submitBatch(signedTxs: string[]) {
  ...
  for (const txCbor of signedTxs) {
    try {
      ...
      const txHash = await sdkClient.submitTx(tx);
...
async function evaluateAndSubmit(signedTxCbor: string) {
  try {
    // Evaluate first
    const signedTx = Transaction.fromCBORHex(signedTxCbor);
    const redeemers = await sdkClient.evaluateTx(signedTx);
